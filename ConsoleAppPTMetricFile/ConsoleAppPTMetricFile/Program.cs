...
namespace ConsoleAppPTMetricFile
{
    class Program
    {
       const string METRIC_FILE_URI = "http://testresults.kymeta.local/Automated/U8/PT_Acquisition_Test";
        ...
        static void Main(string[] args)
        {
            ...
            int jobId = tokJobId == null ? 203 : tokJobId.Value<int>();
            Console.WriteLine("job ID=" + jobId);
            int successJobId = 0;
            int failCount = 0;
            string kPath = "\\\\Kymeta.local\\Shares\\LabData\\staging\\PT_Metric_file\\";
            try
            {
                while (true)
                {
                    string baseUri = $"{METRIC_FILE_URI}/{jobId}";
                  ...
                  //      {
                          ...
                          //  string asm_serial_number = ASM.Value<String>("asm-serial-number");
                            ...
                            string metricFilename = "metric.json";

                            Uri MetricFilenameUri = new Uri($"{baseUri}/{metricFilename}");

                    if (DownloadFile(MetricFilenameUri, jobId.ToString() + "." + "asm_serial_number" + "." + metricFilename, kPath + jobId.ToString() + "." + "asm_serial_number" + "." + metricFilename))
                    {

                        Console.WriteLine("json file: " + Path.GetFullPath(metricFilename));
                        Console.WriteLine("json file: " + kPath + jobId.ToString() + "." +  "asm_serial_number" + "." + metricFilename);
                        successJobId = jobId;
                        failCount = 0;
                    }
                    else
