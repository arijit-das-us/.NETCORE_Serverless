...
namespace ConsoleAppPTXPol
{
    class Program
    {
        const string XPOL_CAL_URI = "http://testresults.kymeta.local/Automated/U8/PT_15_Pointometer";
        ...
        static void Main(string[] args)
        {
            ...
            int jobId = tokJobId == null ? 13113 : tokJobId.Value<int>();
            Console.WriteLine("job ID=" + jobId);
            int successJobId = 0;
            int failCount = 0;
            string kPath = "\\\\Kymeta.local\\Shares\\LabData\\staging\\PT_XPOL\\";
            try
            {
                while (true)
                {
                    string baseUri = $"{XPOL_CAL_URI}/{jobId}";
                    ...
                    if (readSummarySuccess)
                    {
                        ...
                        if (ASM != null)
                        {
                            ...
                            string XpolFilename = "xpol_cal.json";

                            Uri XpolFilenameUri = new Uri($"{baseUri}/{XpolFilename}");

                            if (DownloadFile(XpolFilenameUri, jobId.ToString() + "." + asm_serial_number + "." + XpolFilename, kPath + jobId.ToString() + "." + asm_serial_number + "." + XpolFilename))
                            {

                                Console.WriteLine("json file: " + Path.GetFullPath(XpolFilename));
                                Console.WriteLine("json file: " + kPath + jobId.ToString() + "." + asm_serial_number + "." + XpolFilename);
